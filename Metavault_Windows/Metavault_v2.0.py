...
def form_callback():
    market_tokens = st.session_state['market_tokens']
...
def load_contract():
    ...
    with open(Path('./Metavault_2_abi.json')) as f:
...
#     st.write('')
#     st.write('')       
#     st.write('##### Deploy and link your smart contract before moving to "Marketplace" page..')
...
if choice == "Marketplace":
    ...
    if register_NFT:
        ...
        appraisal_filter = contract.events.registration.createFilter(fromBlock=0, argument_filters={"tokenId": NFT_token_id})
    ...
    if st.button('View Listings'):
        ...
        for i in range(len(NFT_collection)):
            ...
            col.image(values[i][1], width=300)
            col.write(f'Token Id: {NFT_listings[i][0]} | Name :  {NFT_listings[i][1]} | Value: {NFT_listings[i][2]} Eth')
    ...
    market_tokens = token_ids

    if "market_tokens" not in st.session_state:
             st.session_state['market_tokens'] = market_tokens

    st.sidebar.markdown("## Purchase NFT")

    buyer_address = st.sidebar.selectbox("Select Your Account Address", options=accounts)
    selected_NFT = st.sidebar.selectbox("Select the NFT Id You Would Like to Purcahse", options=market_tokens)


    pruchase_NFT = st.sidebar.button("Purchase NFT")

    #  pruchase_NFT = st.sidebar.button("Purchase NFT", on_click=form_callback)
    ...
    if pruchase_NFT:
        ...
        # Get selcted_NFT price
        selected_NFT_price = contract.functions.valueCheck(int(selected_NFT)).call()
        ...
        contract.functions.buyNFT(
            address
        # ).transact({'from': address, 'gas': 1000000})
        ).transact({'from': buyer_address, 'value': selected_NFT_price*10**18,  'gas': 10000000})   

        # Update drop-down menu to only show available tokens

        # for i in range(len(market_tokens)):
        #     if market_tokens[i] == int(selected_NFT):
        #         market_tokens.pop(i)
        #         st.session_state['market_tokens'] = market_tokens
...
# Exchange Page
    ...
    view_address_balance = contract.functions.BalanceCheck(view_address).call() / 10**18
    ...
    if view_NFT:
          ...
          if len(held_NFTs) == 0:
               ...
               st.write(f':black[Balance : {view_address_balance} Eth]')               
          else:
        ...
        #   st.image
          ...
          for i in range(len(held_NFTs)):
            ...
            col.image(NFT_collection[held_NFTs[i]], width=300)

            for nfts in range(len(token_ids)): 
                if held_NFTs[i] == NFT_listings[nfts][0]: 
                    # st.write(NFT_listings[nfts])
                    col.write(f'Token Id: {NFT_listings[nfts][0]} | Name :  {NFT_listings[nfts][1]} | Value: {NFT_listings[nfts][2]} Eth')
                    # col.write(NFT_listings[nfts])                    
            # st.write(f':black[Balance : {view_address_balance} Eth]')
        #   col.write(f':black[Balance : {view_address_balance} Eth]')
    ...
    if Exchange_NFT:
        ...
        contract.functions.buyNFT(
            from_address
        # ).transact({'from': address, 'gas': 1000000})
        ).transact({'from': to_address, 'value': selected_NFT_price*10**18,  'gas': 10000000})   
